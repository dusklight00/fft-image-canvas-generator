...
function createRoundRectangle(context, x, y, width, height, radius) {
  const topX = x - width / 2;
  const topY = y - height / 2;
  createRoundRectangleTopLeftAnchor(context, topX, topY, width, height, radius);
...
function createBottomFixedRoundRectangle(
  context,
  bottomX,
  bottomY,
  width,
  ...
  const middleX = bottomX;
  const middleY = bottomY - height / 2;
  createRoundRectangle(context, middleX, middleY, width, height, 10);
  return context;
}
